<x-layout.main>
    <form action="/post/{{ $post->id }}" method="POST" class="container w-full px-4 py-8 mx-auto">
        ...
        <div class="flex items-center mb-4 ">
            <x-ui.link-btn :mb="0" href="/post/{{ $post->slug }}" :icon="false" label="<<" />
            <h1 class="text-2xl font-bold">Edit Post</h1>
        </div>
        <div class="relative mb-6">
            <input type="text" id="floating_outlined" value="{{ old('title', $post->title) }}" name="title"
        ...
        <div class="relative mb-6">
            <input id="content" value="{{ old('content', $post->content) ?? 'Type your content here' }}"
